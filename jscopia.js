document.addEventListener("DOMContentLoaded", () => {
    ...
    input.addEventListener("input", () => {
      ...
      sectores.forEach(sector => {
        ...
        // Mostrar todos los sectores si no hay filtro, solo los coincidentes si hay filtro
        if (hayFiltro) {
          sector.style.display = sectorTieneCoincidencias ? "block" : "none";
        } else {
          sector.style.display = "block";
          simuladores.forEach(sim => sim.style.display = "block"); // restaurar visibilidad
          simuladoresVisibles = simuladores.length;
        }
  
        // Mostrar flechas solo si hay más de 1 simulador visible
        if (controles) {
